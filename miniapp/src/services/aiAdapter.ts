  ...
  return new Promise((resolve, reject) => {
    uni.request({
      ...
      success: (res) => {
        ...
        const data = res.data as { choices?: { message?: { content?: string } }[]; usage?: { total_tokens?: number } };
        resolve({
  ...
  return new Promise((resolve, reject) => {
    uni.request({
      ...
      success: (res) => {
        ...
        const data = res.data as { content?: { text?: string }[]; usage?: { input_tokens?: number; output_tokens?: number } };
        resolve({
...
async function callGemini(model: AIModel, messages: ChatMessage[]): Promise<{ content: string; tokensUsed: number }> {
  ...
  return new Promise((resolve, reject) => {
    uni.request({
      url: `${model.apiBaseUrl}/models/${model.modelName}:generateContent?key=${model.apiKey}`,
      method: 'POST',
      ...
      success: (res) => {
        ...
        const data = res.data as { candidates?: { content?: { parts?: { text?: string }[] }[] }[] };
        resolve({
...
export async function summarizeWithFallback(
  ...
  const sorted = [...models].sort((a, b) => priorityOrder.indexOf(a.provider) - priorityOrder.indexOf(b.provider));
  const available = sorted.filter((m) => m.apiKey && m.apiKey.trim());

  if (available.length === 0) {
    return { summary: '', keywords: [], tokensUsed: 0, modelId: '', success: false, error: '没有可用的AI模型（请先配置API Key）' };
  }
  ...
  return { summary: '', keywords: [], tokensUsed: 0, modelId: '', success: false, error: `所有模型均失败: ${lastError}` };
}
