/**
 * AI Model Adapter - Web 端
 * 统一 AI 摘要生成适配器，支持多模型优先级切换
 */

import { ModelConfig, SummarizeOptions, SummarizeResult } from '../types';
...
async function callOpenAICompatible(
  ...
  const endpoint = config.provider === 'gemini'
    ? `${config.apiBaseUrl}/models/${config.modelName}:generateContent?key=${config.apiKey}`
